...
public class ProductDBContext extends DBContext {
    ...
    public List<Product> getAllProducts() {
        ...
        try {
            String sql = "select * from product_HE176045";
    ...
    public List<Product> getProductsByCategoryId(int categoryId) {
        List<Product> list = new ArrayList<>();
        try {
            String sql = "select * from Product_HE176045 where Product_HE176045.cateID = ?";
    ...
    public List<Product> getProductsWithPagging(int page, int PAGE_SIZE) {
        List<Product> list = new ArrayList<>();
        try {
            String sql = "select *  from Product_HE176045 order by id\n"
    ...
    public int getTotalProducts() {
        try {
            String sql = "select count(id)  from Product_HE176045 ";
    ...
    public List<Product> search(String keyword) {
        List<Product> list = new ArrayList<>();
        try {
            String sql = "select *  from Product_HE176045 where name like ?";
    ...
    public Product getProductById(int productId) {
        try {
            String sql = "select *  from Product_HE176045 where id = ?";
            PreparedStatement stm = connection.prepareStatement(sql);
...
//Lấy ra danh sách sản phẩm của 1 seller để phục vụ cho chức năng quản lý sản phẩm
    public List<Product> getProductsBySellId(int Id) { 
        List<Product> list = new ArrayList<>();
        try {
            String sql = "select * from Product_HE176045 where sell_ID = ?";
    ...
    public void inSertProduct(Product product) {
        try {
            String sql = "INSERT INTO [product_HE176045]\n"
                    + "           ([name]\n"
                    + "           ,[image]\n"
                    + "           ,[price]\n"
                    + "           ,[title]\n"
    ...
    public void deleteProduct(int id) {
        try {
            String sql = "DELETE FROM [Product_HE176045]\n"
                    + "WHERE id = ? ";
            PreparedStatement stm = connection.prepareStatement(sql);
            stm.setInt(1, id);
            stm.executeUpdate();
    ...
    public void updateProduct(Product p) {
        try {
            String sql = "UPDATE [product_HE176045]\n"
                    + "   SET [name] = ?\n"
                    + "      ,[image] = ?\n"
                    + "      ,[price] = ?\n"
                    + "      ,[title] = ?\n"
    ...
    public List<Product> getAllProductsLast() {
        List<Product> list = new ArrayList<>();
        //truy vấn 4 sản phẩm mới nhất từ bảng,truy vấn 4 sản phẩm mới nhất từ bảng
        try {
            String sql = "SELECT TOP 4 * FROM product_HE176045 ORDER BY ID DESC";
            PreparedStatement stm = connection.prepareStatement(sql);
            ResultSet rs = stm.executeQuery();
            while (rs.next()) {
                Product product = new Product();
